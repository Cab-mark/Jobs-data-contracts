...
if grep -q "schemas:" schemas/jobs/openapi.yaml && grep -A 1 "schemas:" schemas/jobs/openapi.yaml | grep -v "schemas: {}" | grep -qv "^--$"; then
    ...
    datamodel-codegen \
        ...
        --output python/jobs_data_contracts/jobs/models.py \
    ...
    cat > python/jobs_data_contracts/jobs/models.py << 'MODELS_EOF'
...
datamodel-codegen \
    ...
    --input-file-type openapi \
    --output python/jobs_data_contracts/search/models.py \
    --output-model-type pydantic_v2.BaseModel \
    --use-standard-collections \
    --use-schema-description \
    --field-constraints \
